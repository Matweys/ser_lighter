...
class BotApplication:
    ...
    async def _restore_strategies_for_user(self, user_id: int, session: 'UserSession'):
        ...
        try:
            ...
            log_info(user_id, f"Статус автоторговли: {'✅ Активна' if auto_trading_enabled else '❌ Неактивна'}", "BotApplication")

            if not auto_trading_enabled:
                # Если автоторговля неактивна, пользователь не получает никаких действий
                recovery_message = (
                    f"ℹ️ <b>Проверка завершена</b>\n\n"
                    f"Автоторговля отключена. Стратегии не запускаются.\n"
            ...
            # Если у пользователя активна автоторговля, но не было сохранённых стратегий,
            # автоматически запускаем автоторговлю согласно настройкам
            if not restored_strategies:
                log_info(user_id, "Сохранённые стратегии не найдены, но автоторговля активна - запускаю стратегии заново", "BotApplication")

                # Запускаем автоторговлю для этого пользователя
                try:
                    ...
                    if active_strategies_count > 0:
                        recovery_message = (
                            ...
                            f"Сохранённые стратегии не найдены, но автоторговля была активна.\n"
                            f"Запущено стратегий: <b>{active_strategies_count}</b>\n\n"
                            f"✅ Автоторговля возобновлена согласно настройкам."
                        )
                    else:
                        recovery_message = (
                            f"⚠️ <b>Не удалось запустить стратегии</b>\n\n"
                            f"Автоторговля активна, но стратегии не запустились.\n"
                            f"Проверьте настройки или запустите вручную."
                        )

                except Exception as e:
                    log_error(user_id, f"Ошибка автоматического запуска автоторговли: {e}", "BotApplication")
                    recovery_message = (
                        f"❌ <b>Ошибка запуска автоторговли</b>\n\n"
                        f"Не удалось автоматически запустить стратегии.\n"
                        f"Запустите автоторговлю вручную через меню."
                    )
            else:
